    ...
    <DashboardLayout>
      <div className="space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           {kpis.map((kpi, idx) => (
             <div key={idx} className="glass-card p-6 rounded-2xl hover:shadow-xl hover:shadow-cyan-900/20 hover:-translate-y-1 transition-all group overflow-hidden relative border border-white/5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-2 space-y-6">
              <div className="glass-card p-8 rounded-[2rem] relative overflow-hidden border border-white/5">
                <div className="flex items-center justify-between mb-8">
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-slate-950 rounded-xl flex items-center justify-center border border-white/10">
                ...
                <div className="h-64 bg-black/40 rounded-2xl border border-white/5 overflow-y-auto p-4 space-y-3 custom-scrollbar flex flex-col relative group">
                   {logs.length === 0 ? (
                      ...
                      logs.map((log: any) => (
                         <div key={log.id} className="text-xs flex items-start gap-3 p-3 bg-white/5 rounded-lg border border-white/5 relative z-10 w-full transition-colors hover:bg-white/10">
              ...
              <div className="glass-card p-8 rounded-[2rem] border border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="p-5 bg-slate-950 rounded-2xl border border-white/10 flex items-center gap-4">
           ...
           <div className="space-y-6">
              {/* Daily Limits Card */}
              <div className="glass-card p-8 rounded-[2rem] border-l-4 border-l-primary border border-white/5">
                 ...
                 <div className="space-y-6">
                     <div className="space-y-2">
                        ...
                        <div className="h-2 w-full bg-slate-950 rounded-full overflow-hidden border border-white/10">
                           <div className="h-full bg-primary rounded-full transition-all duration-500 shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)]" style={{ width: `${Math.min(100, (stats.messages_sent / 100) * 100)}%` }} />
                        </div>
                     </div>
                     ...
                     <div className="space-y-2">
                        ...
                        <div className="h-2 w-full bg-slate-950 rounded-full overflow-hidden border border-white/10">
                           <div className="h-full bg-blue-500 rounded-full transition-all duration-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" style={{ width: `${Math.min(100, (stats.joined_groups / 15) * 100)}%` }} />
                        </div>
                     </div>
                 </div>
                 ...
                 <div className="mt-8 p-4 bg-primary/5 rounded-xl border border-primary/10">
                    <p className="text-[10px] text-primary/70 font-medium leading-relaxed italic">
                       * "Warm-up Mode" is currently active for all connected clusters.
                    </p>
                 </div>
              </div>
              ...
              <div className="glass-card p-8 rounded-[2rem] border border-white/5">
                 ...
                 <div className="space-y-3">
                    <Link href="/dashboard/campaigns" className="w-full py-4 bg-primary text-white text-[10px] font-black rounded-xl hover:shadow-2xl hover:shadow-primary/30 transition-all uppercase tracking-widest flex items-center justify-center gap-2">
                       <Megaphone className="w-4 h-4" />
                       New Campaign
                    </Link>
                    <Link href="/dashboard/scraper" className="w-full py-4 bg-slate-950 text-foreground text-[10px] font-black rounded-xl hover:bg-slate-900 border border-white/10 transition-all uppercase tracking-widest flex items-center justify-center gap-2">
                       <Search className="w-4 h-4 text-slate-400" />
                       Search Groups
                    </Link>
                    <Link href="/dashboard/templates" className="w-full py-4 bg-slate-950 border border-white/10 text-[10px] font-black rounded-xl hover:bg-slate-900 transition-all uppercase tracking-widest flex items-center justify-center gap-2">
